...
const words = toName
  ? [ {t:toName + ','}, {br:true},
      {t:'You mean,'}, {t:'probably,'}, {br:true},
      {t:'admires', cls:'accent'}, {t:'you.'} ]
  : [ {t:'Happy Birthday'}, {br:true},
      {t:'my beloved doni,'}, {br:true},
      {t:'You mean', cls:'accent'}, {t:'the world', cls:'accent'}, {t:'to me.'} ];
...
if (!reduce) {
  ...
  function triggerBurst(x, y, count = 16) {
    for (let i = 0; i < count; i++) {
      ...
      p.animate([
        ...
        { transform:`translate(${x + dvx}px, ${y + dvy + gravity}px) rotate(${spinDeg}deg)`, opacity:0 }
  ...
  if (candle && flame) {
    candle.addEventListener('click', (e) => {
      ...
      if (wishReveal) wishReveal.classList.add('show');
      const rect = candle.getBoundingClientRect();
      triggerBurst(rect.left + rect.width / 2, rect.top, 28);
    });
  }
}
...
/* ---- gentle background music (autoplay on open + fallback listener + controls) ---- */
(function setupMusic() {
  const btn = document.getElementById('musicToggle');
  const cardBtn = document.getElementById('audioCardPlayBtn');
  ...
  function setState(on) {
    ...
    if (cardBtn) {
      cardBtn.classList.toggle('playing', on);
      const playIcon = cardBtn.querySelector('.play-icon');
      const pauseIcon = cardBtn.querySelector('.pause-icon');
  ...
  function play() {
    audio.play().then(() => { setState(true); fadeTo(TARGET); })
                .catch(() => {
                  // Autoplay blocked by browser policy — show prompt overlay
  ...
  // Start playing immediately when page opens
  play();

  // Instant fallback user interaction handler to guarantee audio plays on first click/tap/scroll anywhere
